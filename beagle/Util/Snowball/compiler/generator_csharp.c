...
static void write_varref(struct generator * g, struct name * p) {
    ...
    /* In csharp, references look just the same */
    write_varname(g, p);
}
...
static void write_hex_plain(struct generator * g, int ch) {
        int i;
        for (i = 12; i >= 0; i -= 4) 
	{
		write_hexdigit(g, ch >> i & 0xf);
	}
}
...
static void generate_repeat(struct generator * g, struct node * p, struct str * loopvar) {
    ...
    g->I[0] = replab;
    w(g, "~Mgoto breplab~I0;~N~}");
    w(g, "~Mbreplab~I0:~N~M;~N");
    str_delete(savevar);
}
...
static void generate_class_begin(struct generator * g) {
    ...
    w(g, "using System.Text;~N"
     	 "#pragma warning disable 162~N"
	 "namespace " BASE_PACKAGE " { ~N"
         "~N"
         "/**~N"
         " * Generated class implementing code defined by a snowball script.~N"
         " */~N"
         "public class ~n : " BASE_CLASS " {~N"
         "~N");
}
...
static void generate_class_end(struct generator * g) {
    ...
    w(g, "~N}");
    w(g, "} // End Namespace ~N~N");
}

static void generate_among_table_ctor(struct generator * g, struct among * x) {
...

    {
	    ...
	    g->I[3] = x->number;
            ...
            w(g, "~Ma_~I3[~I0] = new Among ( ~L0, ~I1, ~I2, \"");
            if (v->function != 0) {
...
static void generate_among_table_decl(struct generator * g, struct among * x) {

    g->I[0] = x->number;
    g->I[1] = x->literalstring_count;

    w(g, "~Mprivate Among[] a_~I0 = new Among[~I1];~N");
}
...
static void generate_grouping_table(struct generator * g, struct grouping * q) {
    ...
    unless (q->no_gaps) {
        ...
        w(g, "~+~+~Mprivate static char[] ~V0 = {");
        for (i = 0; i < size; i++) {
	     write_string(g, "'\\u");
             write_hex_plain(g, map[i]);
	     write_string(g, "'");
             if (i < size - 1) w(g, ", ");
        }
        w(g, " };~N~-~-~N");
...
static void generate_members(struct generator * g) {
    ...
    until (q == 0) {
        ...
        switch (q->type) {
            case t_string:
                w(g, "        private StringBuilder ~W0 = new StringBuilder();~N");
                break;
            case t_integer:
                w(g, "        private int ~W0;~N");
                break;
            case t_boolean:
                w(g, "        private bool ~W0;~N");
                break;
        }
        q = q->next;
    }
